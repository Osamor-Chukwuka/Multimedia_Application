...
export default function App() {
  ...
  //  a state variable to store the search query and update it when the user types in the search input field
  ...
  const createFolder = () => {
    const newFolderPath = filePath ? `${filePath}/${newFolderName}` : newFolderName;
    ...
    // Update the myFiles state with the new folder
    setMyFiles([...myFiles, newFolder]);

    // Reset the new folder name and close the prompt
    setNewFolderName("");
        ...
        <div style={styles.container}>
          ...
          <div style={styles.fileContainer}>
            <div style={{ width: "100%", padding: 10 }}>
              {/* Modify the rendering of the file list to include the search functionality */}
              {myFiles
                .filter(file => file.path && file.path.slice(0, filePath.length) === filePath)
